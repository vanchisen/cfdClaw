...
OUTPNG = BASE / 'most_unstable_frequency_vs_x_Re5K_Re11K.png'
OUTCSV = BASE / 'most_unstable_frequency_vs_x_Re5K_Re11K.csv'
...
def main():
    ...
    plt.plot(df5['x'], df5['frequency'], 'o-', lw=1.8, ms=4, label='Re5K')
    plt.plot(df11['x'], df11['frequency'], 's-', lw=1.8, ms=4, label='Re11K')
    plt.xlabel('x')
    plt.ylabel(r'Most unstable frequency, Re($\omega$)/(2$\pi$)')
    plt.title('Most unstable temporal frequency vs x')
